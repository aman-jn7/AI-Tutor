...
async def start():
    await cl.Message(
        ...
        "\n\nYou can also type **'askat'** to enter Ask AI Tutor mode.").send()
...
async def main(message: cl.Message):
    ...
    if topic in ["askat","ask at"]:
        await cl.Message(content="💬 **AskAT Mode Activated!**\nAsk me anything related to any topic or type **'exit'** to leave AskAT mode.").send()
        while True:
            user_input = await cl.AskUserMessage(content="🧠 Ask me anything:").send()
            question = user_input.get("output", "").strip()
            if question.lower() in ["exit", "quit", "bye"]:
                await cl.Message(content="👋 Exiting AskAT mode. \n\n" \
                "Please enter the topic you want to learn about, and I will generate a full study plan, explanation, quiz, and useful learning links just for you. 🧠📚\n\n**Enter a topic to get started!**" \
                "\n\nYou can also type **'askat'** to enter Ask AI Tutor mode. Type 'exit' to leave AskAT mode.").send()
                break
            messages = [
                SystemMessage(content="You are **AI Tutor Team**, a friendly and knowledgeable study assistant. Always refer to yourself as AI Tutor."),
